...
function App() {
  ...
  // --- Duration Calculation Logic (Improved) ---
  const calculateDuration = (inTime, outTime) => {
    if (!inTime || !outTime) return "-";
    try {
      const parseTime = (timeStr) => {
        // Regex: Extracts hours, minutes, and AM/PM
  ...
  const handleAttendance = async (actionType) => {
    ...
    const currentTime = now.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', hour12: true });
    ...
    try {
      ...
      if (actionType === 'ClockIn') {
        if (existingRecord && existingRecord.ClockIn) {
          showAlert(`${selectedName} သည် ယနေ့အတွက် Clock In လုပ်ပြီးသားဖြစ်သည်`, "warning");
          setLoading(false);
          return;
        }
      }
      ...
      if (actionType === 'ClockOut') {
        ...
        if (existingRecord.ClockOut) {
          showAlert("ယနေ့အတွက် Clock Out လုပ်ပြီးသားဖြစ်သည်", "warning");
          setLoading(false);
          return;
        }
      }
      ...
      if (existingIdx !== -1) {
        let updateData = {};
        if (actionType === 'ClockOut') {
          const duration = calculateDuration(existingRecord.ClockIn, currentTime);
          updateData = { ClockOut: currentTime, Duration: duration };
        } else {
          updateData = { ClockIn: currentTime };
        }
        ...
        await axios.post(API_URL, {
          Name: selectedName, 
          Date: todayStr,
          ClockIn: actionType === 'ClockIn' ? currentTime : '',
          ClockOut: actionType === 'ClockOut' ? currentTime : '',
          Duration: ''
        });
      }
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <p style={styles.subtitle}>Office Hours: 9:00 AM - 5:00 PM</p>
      </div>
...
const styles = {
  container: { padding: '50px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', backgroundColor: '#f0f4f8', minHeight: '100vh', fontFamily: "'Segoe UI', sans-serif" },
  ...
  logoCircle: { width: '90px', height: '90px', borderRadius: '50%', backgroundColor: '#fff', margin: '0 auto 15px', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' },
  logoImg: { width: '100%', height: 'auto' },
  mainTitle: { margin: 0, color: '#1e293b', fontSize: '32px', fontWeight: '800' },
  ...
  select: { width: '100%', padding: '16px', borderRadius: '16px', border: '2px solid #e2e8f0', fontSize: '16px' },
  ...
  tableCard: { width: '100%', maxWidth: '900px', backgroundColor: '#fff', padding: '30px', borderRadius: '32px' },
  tableHeaderSection: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px' },
  tableTitle: { margin: 0, fontSize: '20px' },
  refreshBtn: { border: '1px solid #e2e8f0', background: '#fff', padding: '10px 18px', borderRadius: '12px', cursor: 'pointer' },
  tableWrapper: { overflowX: 'auto' },
  table: { width: '100%', borderCollapse: 'separate', borderSpacing: '0 8px' },
  th: { textAlign: 'left', padding: '15px', color: '#94a3b8', fontSize: '12px' },
  ...
  durationBadge: { backgroundColor: '#3b82f6', color: '#fff', padding: '6px 12px', borderRadius: '10px', fontSize: '13px' },
